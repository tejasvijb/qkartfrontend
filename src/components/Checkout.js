...
const AddNewAddressView = ({
  displayAddressView,
  setDisplayAddressView,
  token,
  newAddress,
  handleNewAddress,
  addAddress,
}) => {
  // if(!displayAddressView) return null
  return (
...
const Checkout = () => {
  ...
  const [displayAddressView, setDisplayAddressView] = useState(false);
   ...
   *
   */
  const validateRequest = (items, addresses, newAddress) => {
    const balance = localStorage.getItem("balance");
    const totalCartValue = getTotalCartValue(items);
    if (parseInt(balance) < totalCartValue) {
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
        { variant: "warning" }
      );
      ...
      let remainingBalance = parseInt(balance) - totalCartValue;
      localStorage.setItem("balance", remainingBalance.toString());
    }
    ...
    if (newAddress.isAddingNewAddress) {
      enqueueSnackbar("Please click Add or Cancel to proceed.", {
        variant: "warning",
      });
      return false;
    }
   ...
   *
   */
  const performCheckout = async (token, items, addresses, newAddress) => {
    if (!validateRequest(items, addresses, newAddress)) return;
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses.all.length === 0 ? (
                ...
                addresses.all.map(({ address, _id }) => {
                  return (
                    <Box
                      key={_id}
                      fullwidth="true"
                      className={`address-item ${
                        addresses.selected === _id ? "selected" : "not-selected"
                      }`}
                      onClick={() =>
                    ...
                    >
                      ...
                      <Button onClick={() => deleteAddress(token, _id)}>
            ...
            {displayAddressView ? (
              <AddNewAddressView
                setDisplayAddressView={setDisplayAddressView}
                // setNewAddress={setNewAddress}
                displayAddressView={displayAddressView}
                token={token}
            ...
            <Button
              ...
              variant="contained"
              onClick={() =>
                performCheckout(token, items, addresses, newAddress)
              }
            >
              PLACE ORDER
            </Button>
